 ...
 */
prove(L => R):- 
    member(A -> B, R), % Find if A -> B exists on right
    del(A -> B, R, NewR), % Delete A -> B on the right and store previous right in NewR
    nl, write('=\t'), write([A | L] => [B | NewR]), % Print the new formula 
    write('\t (implication right)'), % Print the type of change
    prove([A | L] => [B | NewR]). % Recursion call 
